...
public:
  ...
  */
   ...
   * while in a menu: the stream format is unchanged, so the running decoders
   * can be reused (no close/reopen -> no DV re-latch/toast/black between menu
   * loop segments). Cleared by any playlist-scope event (playlist/title/seek). */
