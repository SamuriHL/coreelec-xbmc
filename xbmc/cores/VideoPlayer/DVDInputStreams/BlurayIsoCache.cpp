...
void CBlurayIsoCache::QueueForwardPrefetch(int64_t firstPage, int64_t lastPage)
{
  ...
  QueuePrefetchWindow(queueFrom, static_cast<size_t>(windowEnd - queueFrom + 1));
}

void CBlurayIsoCache::QueuePrefetchWindow(int64_t firstPage, size_t pageCount)
{
  if (pageCount == 0)
    return;

  for (size_t i = 0; i < pageCount; ++i)
    QueuePage(firstPage + static_cast<int64_t>(i));
}

void CBlurayIsoCache::QueuePage(int64_t pageIndex)
{
...

  {
    ...
    // mechanism: the window was fed forwards, so it arrived reversed and the
    // worker started at the far end of a 32 MiB window while the player missed
    // on the very next page and fell back to synchronous reads. Nor is
    // reversing the feed enough, because QueueForwardPrefetch throttles to the
    // newly exposed edge once the horizon is armed: from then on every call
    // queues exactly ONE page, the farthest, which push_front would keep
    // inserting ahead of the entire un-drained near backlog.
  }
